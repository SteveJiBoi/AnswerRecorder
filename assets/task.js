...
(function () {
  ...
  function fillQuestion(sec, modIdx, item, dubious) {
    ...
    const desc = {
      ...
      key: item.ans || null,          // always keep the doc's answer (fragment) for display
      nograde: dubious || !item.ans,  // dubious fills are shown as reference but not scored
      extra: false,
      el: q,
  ...
  function renderReading() {
    ...
    task.reading.forEach((mod, mi) => {
      ...
      if (mod.dubiousFills) {
        ...
        b.textContent = "⚠ 该卷填空答案在原文档中只有词尾片段（如「ther」「rature」），已作为参考答案显示，但不计入自动批改分数（选择题正常批改）。";
        box.appendChild(b);
      }
  ...
  function gradeSection(sec) {
    ...
    Q[sec].forEach(d => {
      ...
      if (d.extra) {
        ...
        // not scored, but show the document's answer as a reference if we have one
        ...
        if (d.key) {
          ...
          if (d.type === "mc") {
            d.el.querySelectorAll(".opt").forEach(o => {
              ...
              if (o.querySelector("input").value === d.key.toLowerCase()) o.classList.add("key");
            });
            line.innerHTML = `<span class="key">参考答案 ${d.key.toUpperCase()}（不计分）</span>` + (stu ? ` · 你的答案 ${stu.toUpperCase()}` : "");
          } else {
            line.innerHTML = `<span class="key">参考答案「${esc(d.key)}」（不计分）</span>` + (stu ? ` · 你的答案「${esc(stu)}」` : "");
          }
        } else {
          result = d.type === "fill" ? "未批改(答案缺失)" : "答案缺失(不批改)";
        }
      } else {
        gradeable++;
        const ok = d.type === "mc"
      ...
      rows.push({ sec, mod: d.mod + 1, n: d.n, type: d.type === "mc" ? "选择" : "填空", student: stu, key: d.key || "", result, note: d.getNote() });
    });
  ...
  function exportExcel() {
    ...
    // 错题 sheet: reading + listening questions marked 错误 or 未作答
    const wrongHeader = ["部分", "模块", "题号", "题型", "你的答案", "正确答案", "结果", "备注"];
    const wrongRows = [wrongHeader];
    r.rows.concat(l.rows).forEach(row => {
      if (row.result === "错误" || row.result === "未作答") {
        wrongRows.push([row.sec === "reading" ? "阅读" : "听力", row.mod, row.n, row.type, row.student, row.key, row.result, row.note]);
      }
    });
    if (wrongRows.length === 1) wrongRows.push(["—", "", "", "", "", "", "无错题（全部正确或无可批改题目）", ""]);
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(wrongRows), "错题");
